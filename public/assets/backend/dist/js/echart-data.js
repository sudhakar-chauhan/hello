...
var echartsConfig = function() { 
	...
	if($('#order_status_chart').length > 0){
		var chartDom = document.getElementById('order_status_chart');

		let pending = $('#pendingOrder').data('value');
		let processing = $('#processingOrder').data('value');
		let shipped = $('#shippedOrder').data('value');
		let delivered = $('#deliverdOrder').data('value');
   var order_status_chart = echarts.init(chartDom);
   var option;
...
option = {
  ...
  color: ['#e5e5e5', '#94660c','#2196F3','#c6e1c6'],	
  series: [
    {
      name: 'Access From',
      type: 'pie',
...
option && order_status_chart.setOption(option);
	}
}
